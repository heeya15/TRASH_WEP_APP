<?php
	 ...
	 $map_y=$_POST["map_y"];
 	 ...
 	 //db 에저장 
 	 $yoil = get_yoil(date("Y-m-d"));  //functions.php 에서 선언한 함수호출
 	 $time = date("H");
	 $date = date("Y-m-d H:i:s");
  	 $sql = "INSERT INTO `garbage` (`mb_id`,`file_name`,`garbage_type`, `addr`,`region1`,`region2`,`region3`,`map_x`,`map_y`,`memo`,`regdate`,`yoil`,`time`) VALUES ('".$_SESSION["ss_id"]."','".$original_file_name."','".$garbage_type."', '".$addr."','".$region1."','".$region2."','".$region3."','".$map_x."','".$map_y."', '".$memo.'$date'.$yoil."','".$time."');";
